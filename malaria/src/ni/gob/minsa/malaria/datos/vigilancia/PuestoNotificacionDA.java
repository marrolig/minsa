...
public class PuestoNotificacionDA implements PuestoNotificacionService {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<DivisionPolitica> ListarMunicipiosPorEntidad(
			long pEntidadAdtvaId, boolean pSoloActivos) {
		EntityManager em = jpaResourceBean.getEMF().createEntityManager();
	    try{
	    	Query query = null;
	    	query=em.createNamedQuery("PuestoNotificacion.listarMunicipiosPorEntidad");
	    	query.setParameter("pEntidadAdtvaId",pEntidadAdtvaId);
	    	query.setParameter("pTodos", pSoloActivos?0:1);
            query.setHint("javax.persistence.cache.storeMode", "REFRESH");
	        return(query.getResultList());
